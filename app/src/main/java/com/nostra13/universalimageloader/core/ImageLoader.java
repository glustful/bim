...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		BitmapResult bitmapResult = configuration.memoryCache.get(memoryCacheKey);
		Bitmap bmp = null;
	 ...
	 * @param options
	 */
	public void displayImage(String nameSpace,String methodName,String uri,String soapHeader,HashMap<String,String> params,HashMap<String,String> header,ImageView imageAware, DisplayImageOptions options){
		displayImage(nameSpace,methodName,uri,soapHeader,params,header,imageAware,options,null,null,null);
	}
	 ...
	 */
	public void displayImage(String nameSpace,String methodName,String url,String soapHeader,HashMap<String,String> params,HashMap<String,String> header,ImageView imageView, DisplayImageOptions options,
							 ImageSize targetSize, ImageLoadingListener listener, ImageLoadingProgressListener progressListener) {
		checkConfiguration();
		...
		//uri += "/" + System.currentTimeMillis()+".jpg";
		if (TextUtils.isEmpty(uri)) {
			engine.cancelDisplayTaskFor(imageAware);
			listener.onLoadingStarted(uri, imageAware.getWrappedView());
			if (options.shouldShowImageForEmptyUri()) {
		...
		if (bmp != null && !bmp.isRecycled()) {
			L.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);
			imageView.setTag(R.id.image_loader_photo_size,bitmapResult.getImageSize());
			if (options.shouldPostProcess()) {
				ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
						options, listener, progressListener, engine.getLockForUri(uri));
				ProcessAndDisplayImageTask displayTask = new ProcessAndDisplayImageTask(engine, bmp, imageLoadingInfo,
			...
			LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(engine, imageLoadingInfo,
					defineHandler(options));
			displayTask.setWebService(nameSpace,methodName,url,soapHeader,params,header);
			if (options.isSyncLoading()) {
				displayTask.run();
			} else {
				engine.submit(displayTask);
