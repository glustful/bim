...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private ByteDownloader webserviceImageDownloader;
	...
	public void setWebService(String nameSpace,String methodName,String url,String soapHeader,HashMap<String,String> params,HashMap<String,String> header){
		this.isWebService = true;
		webserviceImageDownloader = new ByteDownloader(nameSpace,methodName,url,soapHeader,params,header);
	}
	...
	public void run() {
		...
		try {
			checkTaskNotActual();
			 ...
			 bitmapResult = configuration.memoryCache.get(memoryCacheKey);
			if (bitmapResult!=null)
				bmp = bitmapResult.getBitmap();
			if (bmp == null || bmp.isRecycled()) {
				bitmapResult = tryLoadBitmap();
				if (bitmapResult!=null)
					bmp = bitmapResult.getBitmap();
				...
				if (bmp != null && options.isCacheInMemory()) {
					...
					configuration.memoryCache.put(memoryCacheKey, bitmapResult);
				}
			} else {
		...
		if (bitmapResult != null){
			imageLoadingInfo.imageAware.getWrappedView().setTag(R.id.image_loader_photo_size,bitmapResult.getImageSize());
		}
	...
	private BitmapResult tryLoadBitmap() throws TaskCancelledException {
		Bitmap bitmap = null;
		...
		try {
			...
			if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
				...
				checkTaskNotActual();
				bitmapResult = decodeImage(Scheme.FILE.wrap(imageFile.getAbsolutePath()));
				if (bitmapResult != null)
	...
	private BitmapResult decodeImage(String imageUri) throws IOException {
	...
	private boolean downloadImage() throws IOException {
		if (isWebService){
			byte[] data = webserviceImageDownloader.download();
			return configuration.diskCache.save(uri,data);
		}
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		if (targetFile != null && targetFile.exists()) {
			...
			Bitmap bmp = decoder.decode(decodingInfo).getBitmap();
